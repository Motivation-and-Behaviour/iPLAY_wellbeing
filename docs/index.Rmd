...
The result of the unadjusted model is shown in Table \@ref(tab:resultstable).
A significant moderation effect of baseline wellbeing was observed at 24 months.
Including the covariates did not change the results, so the remaining results are drawn from the unadjusted model.

```{r resultstable}
results_table$unadjusted
```
...
* There's a strong effect of the intervention on students with very low wellbeing, mostly at 24 months (the difference at 12m is not significant).
